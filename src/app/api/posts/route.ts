...
export async function GET(request: NextRequest) {
  try {
    ...
    const countRow = await db.get(countQuery, ...(categoryId ? [categoryId] : [])) as { total: number } | undefined;
    ...
    return NextResponse.json({ error: error.message, posts: [], total: 0, page: 1, totalPages: 0 }, { status: 500 });
  }
}
